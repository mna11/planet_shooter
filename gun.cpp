...
void GUN::randomBulletTexture() // 총알 10개를 세팅해준다. 
{
  ...
  for(int i = 0; i < 10; i++){
    ...
    int random_texture_number = rand() % 3;
    while (!remain_texture[random_texture_number]) random_texture_number = rand() % 3;
    bullet_texture_shape[i].setTexture(&textures[texture_number[i] = random_texture_number]); // Textures를 정한다. -> 3개의 Textures 중 랜덤으로 
...
void GUN::changeTexture(NPC_SET& _npcs)
{
  if (bullet_now == 10) { 
    remain_texture = _npcs.remainTexture();
    randomBulletTexture(); // 10개를 다 쓴 경우, 다시 리로드를 위해 randomBulletTexture()을 부른다. 
    return;
  }
  current_texture = *(bullet_texture_shape[bullet_now++].getTexture());
  gun.setTexture(&current_texture);
